...
public:
    typedef struct {
        float latencySeconds;      // Current latency in seconds
        uint32_t bufferBytes;      // Bytes currently in buffer
        uint32_t bufferSize;       // Total buffer capacity in bytes
        float bufferPercent;       // Buffer fill percentage (0-100)
        uint32_t bitrate;          // Current bitrate in bps
        float bytesPerSecond;      // Bytes consumed per second
        uint32_t timeRemaining;    // Seconds of audio remaining in buffer
    } BufferInfo;
