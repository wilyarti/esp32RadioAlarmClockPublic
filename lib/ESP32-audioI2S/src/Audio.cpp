...
Audio::BufferInfo Audio::getBufferInfo() {
    BufferInfo info;

    // Get current bitrate
    info.bitrate = getBitRate(true);

    // Get buffer statistics
    info.bufferBytes = inBufferFilled();
    info.bufferSize = inBufferFree() + info.bufferBytes; // Total capacity

    // Calculate latency and related metrics
    if (info.bitrate > 0) {
        info.latencySeconds = (float)(info.bufferBytes * 8) / info.bitrate;
        info.bytesPerSecond = (float)info.bitrate / 8;
        info.timeRemaining = (uint32_t)info.latencySeconds;
    } else {
        info.latencySeconds = 0;
    ...
    if (info.bufferSize > 0) {
        info.bufferPercent = (float)info.bufferBytes / info.bufferSize * 100;
    } else {
        info.bufferPercent = 0;
    }
...
//---------------------------------------------------------------------------------------------------------------------
float Audio::getStreamLatency() {
    if (getBitRate(true) > 0) {
        return (float)(inBufferFilled() * 8) / getBitRate(true);
    }
    return 0;
}
//---------------------------------------------------------------------------------------------------------------------
